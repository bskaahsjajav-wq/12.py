...
# ==========================================
#  HỆ THỐNG MÀU SẮC GRADIENT 7 MÀU (TU TIÊN)
# ==========================================
RESET = "\033[0m"
...
class MQTTManager:
    ...
    def connect(self):
        try:
            ...
            print_color("🔄 Đang khai mở linh mạch...", "info")
            self.mqtt.connect(
    ...
    def send_typing(self, thread_id, is_typing=True):
        ...
        try:
            ...
            self.mqtt.publish("/ls_req", json.dumps(content, separators=(",", ":")), qos=1, retain=False)
            return True
        except Exception as e:
...
class Messenger:
    ...
    def connect_mqtt(self):
        try:
            ...
            if self.mqtt_manager.connect():
                print_color("✅ Zin Thiên Đạo - Linh Mạch Đã Thông", "success")
                return True
            else:
    ...
    def get_thread_list(self, limit=100):
        ...
        try:
            ...
            return {"success": True, "thread_count": len(thread_list), "threads": thread_list}
    ...
    try:
        ...
        while True:
            for thread_id, thread_name in zip(thread_ids, thread_names):
                ...
                print_color(f"👤 Tu sĩ: {account_name} | 📦 Động phủ: {thread_name} | 🏷️ Triệu hồi: {len(tag_names)} người | 📊 Kết quả: {status_text}", status_color)
...
def start_multiple_accounts():
    clear()
    print_banner()
    print_header("𝙕𝙄𝙉 𝙏𝙃𝙄Ê𝙉 ĐẠO - 𝙑Ĩ𝙉𝙃 𝙃Ằ𝙉𝙂 𝘾𝙃Í 𝙏Ô𝙉")
    try:
    ...
    for i in range(num_accounts):
        ...
        if cl["status"] == "success":
            print_color(f"✅ Tu sĩ: {cl['name']} (ID: {cl['user_id']}) - Linh thức ổn định!", "success")
        else:
        ...
        try:
            ...
            if not threads_list:
                print_color("❌ Không tìm thấy động phủ nào. Bỏ qua tài khoản này.", "error")
                continue

            table = Table(title=f"📦 DANH SÁCH ĐỘNG PHỦ - {len(threads_list)}", show_header=True, header_style="bold magenta", box=box.ROUNDED)
            ...
            raw = input(gradient_text("🎯 Chọn động phủ (VD: 1,3 hoặc all): ", [(0,255,0), (0,0,255)])).strip()
            if raw.lower() == 'all':
                selected = list(range(1, len(threads_list) + 1))
            else:
                selected = parse_selection(raw, len(threads_list))
            if not selected:
                print_color("❌ Không chọn động phủ nào! Bỏ qua tài khoản này.", "error")
                continue
            ...
            print_color(f"\n🔄 Đang triệu hồi danh sách đệ tử...", "info")
            members = []
            for thread_id in selected_ids:
                ...
                if result.get("success"):
                    ...
                    print_color(f"⚠️ Lỗi lấy thành viên cho động phủ {thread_id}: {result['error']}", "warning")

            if not members:
                print_color("❌ Không tìm thấy đệ tử nào trong các động phủ đã chọn. Bỏ qua tài khoản này.", "error")
                continue

            member_table = Table(title=f"👥 DANH SÁCH ĐỆ TỬ - {len(members)}", show_header=True, header_style="bold blue", box=box.ROUNDED)
            ...
            raw_tags = input(gradient_text("🏷️ Chọn đệ tử để triệu hồi (VD: 1,2,3 hoặc all) hoặc 'khong' để bỏ qua: ", [(0,255,0), (0,0,255)])).strip()
            tag_ids = []
            tag_names = []
            if raw_tags.lower() != 'khong':
                ...
                if not selected_tags:
                    print_color("❌ Không chọn đệ tử nào để triệu hồi! Bỏ qua tài khoản này.", "error")
                    continue
                tag_ids = [members[i - 1]['id'] for i in selected_tags]
                tag_names = [members[i - 1]['name'] for i in selected_tags]
                print_color(f"✅ Đã chọn {len(tag_ids)} đệ tử để triệu hồi", "success")

            file_txt = input(gradient_text("📂 Nhập tên file .txt chứa pháp quyết (nội dung): ", [(0,255,0), (0,0,255)])).strip()
            try:
                message_lines = load_file(file_txt)
                print_color(f"✅ Đã tải {len(message_lines)} dòng pháp quyết từ {file_txt}", "success")
            except Exception as e:
            ...
            try:
                delay = int(input(gradient_text("⏳ Nhập thời gian hồi phục linh lực (giây): ", [(0,255,0), (0,0,255)])))
                if delay < 1:
                    print_color("❌ Thời gian phải là số nguyên dương. Bỏ qua tài khoản này.", "error")
                    continue
            except ValueError:
                print_color("❌ Thời gian phải là số nguyên. Bỏ qua tài khoản này.", "error")
                continue

            print_header(f"🚀 KHAI MỞ PHÁP TRẬN - {cl['name']}")
            if tag_ids:
                print_color(f"🎯 Triệu hồi {len(tag_ids)} đệ tử: {', '.join(tag_names[:3])}{'...' if len(tag_names) > 3 else ''}", "cyan")
            if messenger.mqtt_manager and messenger.mqtt_manager.connected:
                print_color("⚡ Bắt đầu vận chuyển linh khí", "success")
            else:
                print_color("⚠️ Sử dụng phương thức truyền tin thường", "warning")
    ...
    print_header("🎉 PHÁP TRẬN ĐÃ KHAI MỞ")
    print_color(f"✅ Đã khởi động {len(processes)} tài khoản", "success")
    print_color("⚡ Tính năng: Triệu hồi thật + Giả lập gõ phím", "cyan")
