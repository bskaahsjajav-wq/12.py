...
Version 6.5 - Bỏ reload & Bỏ cảnh báo DIE ảo
"""
...
{C['dim']}  ⚡ Version 6.5 | 🔱 Bỏ reload & Bỏ cảnh báo DIE ảo{C['reset']}
""")
...
def run_report(driver, report):
    ...
    if had_sorry:
        # BỎ PHẦN SET TARGET_DIED THEO YÊU CẦU
...
def start_firefox():
    ...
    opts = Options()
    opts.add_argument("--headless") # <--- ĐÃ THÊM VÀO ĐÂY
    ...
    try:
        ...
        try:
            while True:
                ...
                # ===== ĐÃ BỎ TOÀN BỘ PHẦN KIỂM TRA VÀ RELOAD =====
                log(f"Nghỉ {CYCLE_REST}s rồi tự động chạy vòng tiếp theo...", "yellow", "💤")
                sleep(CYCLE_REST)
