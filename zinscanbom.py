...
# ASCII spinner — Termux không hỗ trợ braille
SPINNER  = ["|", "/", "-", "\\"]
...
def _rong_terminal():
    """Detect terminal width — ưu tiên env COLUMNS (Termux luôn có)."""
    try:
...
def _rong_khung():
    """Chiều rộng khung tối ưu (chừa 4 cột lề)."""
    tw = _rong_terminal()
...
def _cat_theo_rong(s, toi_da):
    """Cắt chuỗi có ANSI để hiển thị <= toi_da cột."""
...
# ═══════════════════════════════════════════════════════════
# LOGO ANIMATION VÔ HẠN
# ═══════════════════════════════════════════════════════════
def in_logo_vo_han():
    """Animation logo chạy LIÊN TỤC đến khi user nhấn Enter."""
...
# ═══════════════════════════════════════════════════════════
# SCAN MODES — áp dụng cho mọi mode
# ═══════════════════════════════════════════════════════════
SCAN_MODES = {
    "1": {
        "name": "SCAN NHANH",
...
class ThongKe:
    ...
    def dong_trang_thai(self):
        ...
        return (
            ...
            f"{C.GRAY}|{C.R}"
            f"{mau(2)}{C.B}2F:{self.hai_buoc}{C.R}"
            f"{C.GRAY}|{C.R}"
            f"{mau(3)}{C.B}E:{self.loi_request}{C.R}"
            f"{C.GRAY}|{C.R}"
...
class BoKiemTra:
    ...
    def _hop(self, mau, tieu_de, uid, mat_khau, proxy):
        ...
        try:
            with _STDOUT_KHOA:
                # Xoá dòng status cũ bằng space (Termux-safe)
...
class UngDungZin:
    ...
    async def _chay_bat_dong_bo(self, ten, tien_to, do_dai, so_luong,
        ...
        thong_ke      = ThongKe()
        thong_ke.uid_tong = so_luong   # ← set tổng UID cho mọi mode
    ...
    def _in_tong_ket(self, thong_ke, ghi_file, da_sinh,
        ...
        print(f"  {C.CYAN}{C.B}UID Đã Xử Lý    {C.R}{C.GRAY}▸{C.R} "
              f"{C.CYAN}{C.B}{thong_ke.uid_da_xu_ly}/{thong_ke.uid_tong}{C.R}")
